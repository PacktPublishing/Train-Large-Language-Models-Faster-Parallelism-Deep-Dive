...
def test_predictions(model, test_dataloader, raw_test_dataset, num_samples=10):
    """Generate predictions for visualization and console output with test phrases"""
    model.eval()
    all_preds = []
    all_labels = []
    samples = []
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_dataloader):
            ...
            if i == 0:
                # Get the original text for the first batch
                for j in range(min(num_samples, len(input_ids))):
                    # Map batch index to original dataset index
                    dataset_idx = i * test_dataloader.batch_size + j
                    if dataset_idx < len(raw_test_dataset):
                        original_text = raw_test_dataset[dataset_idx]['text'][:50] + "..."  # Limit to 50 chars for brevity
                        samples.append({
                            'text': original_text,
                            'label': labels[j].item(),
    ...
    print(f"True Negatives (Actual Negative, Predicted Negative): {cm[0][0]}")
    print(f"False Positives (Actual Negative, Predicted Positive): {cm[0][1]}")
    print(f"False Negatives (Actual Positive, Predicted Negative): {cm[1][0]}")
    print(f"True Positives (Actual Positive, Predicted Positive): {cm[1][1]}")
    print("\nSample Predictions with Test Phrases (First 10):")
    for i, sample in enumerate(samples[:10]):
        true_label = 'Positive' if sample['label'] == 1 else 'Negative'
        pred_label = 'Positive' if sample['prediction'] == 1 else 'Negative'
        color = '✓' if sample['label'] == sample['prediction'] else '✗'
        print(f"Sample {i+1}: Text = '{sample['text']}', True = {true_label}, Predicted = {pred_label} [{color}]")
...
def main():
    ...
    test_dataset = imdb_dataset['test']  # Raw test dataset
    ...
    transformed_test_dataset = IMDBForTransformer(test_dataset, tokenizer, max_len=args.max_len)
    
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2)
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=2)
    test_dataloader = DataLoader(transformed_test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=2)
    print(f"Dataset loaded: Train={len(train_dataset)}, Val={len(val_dataset)}, Test={len(transformed_test_dataset)}")
    ...
    test_results = test_predictions(model, test_dataloader, test_dataset)  # Pass raw test_dataset here
    ...
    print(f"Confusion Matrix Summary: {test_results['confusion_matrix']}")
    print(f"Sample Predictions (First 10) were printed earlier in the test predictions output.")
